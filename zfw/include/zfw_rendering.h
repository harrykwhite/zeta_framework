...
typedef struct
{
    zfw_render_layer_char_batch_bits_t
        batch_init_bits[ZFW_RENDER_LAYER_LIMIT]; // Each bit represents whether the corresponding batch has been
                                                 // initialized.
...
zfw_bool_t zfw_write_to_render_layer_char_batch(
    const zfw_char_batch_key_t key, const char *const text, const zfw_font_hor_align_t hor_align,
    const zfw_font_vert_align_t vert_align, zfw_char_batch_group_t *const batch_group,
    const zfw_user_font_data_t
        *const user_font_data); // IDEA: Have a faster alternative to this to be used when no alignment is needed.
...
{
    return (layer_index * ZFW_RENDER_LAYER_SPRITE_BATCH_LIMIT * ZFW_SPRITE_BATCH_SLOT_LIMIT) +
           (batch_index * ZFW_SPRITE_BATCH_SLOT_LIMIT) + slot_index;
}

inline int zfw_get_sprite_batch_group_tex_unit_index(const int layer_index, const int batch_index,
                                                     const int tex_unit_index)
{
    return (layer_index * ZFW_RENDER_LAYER_SPRITE_BATCH_LIMIT * ZFW_SPRITE_BATCH_TEX_UNIT_LIMIT) +
           (batch_index * ZFW_SPRITE_BATCH_TEX_UNIT_LIMIT) + tex_unit_index;
}
...
inline void zfw_set_render_layer_char_batch_user_font_index(const zfw_char_batch_key_t key, const int user_font_index,
                                                            zfw_char_batch_group_t *const batch_group)
{
    batch_group->user_font_indexes[zfw_get_char_batch_group_batch_index(
        zfw_get_char_batch_slot_key_layer_index(key), zfw_get_char_batch_slot_key_batch_index(key))] = user_font_index;
}

inline void zfw_set_render_layer_char_batch_pos(const zfw_char_batch_key_t key, const zfw_vec_2d_t pos,
                                                zfw_char_batch_group_t *const batch_group)
{
    batch_group->positions[zfw_get_char_batch_group_batch_index(zfw_get_char_batch_slot_key_layer_index(key),
                                                                zfw_get_char_batch_slot_key_batch_index(key))] = pos;
}

inline void zfw_set_render_layer_char_batch_scale(const zfw_char_batch_key_t key, const zfw_vec_2d_t scale,
                                                  zfw_char_batch_group_t *const batch_group)
{
    batch_group->scales[zfw_get_char_batch_group_batch_index(zfw_get_char_batch_slot_key_layer_index(key),
                                                             zfw_get_char_batch_slot_key_batch_index(key))] = scale;
}

inline void zfw_set_render_layer_char_batch_blend(const zfw_char_batch_key_t key, const zfw_color_t *const blend,
                                                  zfw_char_batch_group_t *const batch_group)
{
    batch_group->blends[zfw_get_char_batch_group_batch_index(zfw_get_char_batch_slot_key_layer_index(key),
                                                             zfw_get_char_batch_slot_key_batch_index(key))] = *blend;
}

inline zfw_vec_2d_t zfw_get_view_to_screen_pos(const zfw_vec_2d_t pos, const zfw_view_state_t *const view_state)
{
    return zfw_get_vec_2d_scaled(zfw_create_vec_2d(pos.x - view_state->pos.x, pos.y - view_state->pos.y),
                                 view_state->scale);
}
