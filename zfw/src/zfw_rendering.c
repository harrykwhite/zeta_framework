...
static zfw_bool_t init_and_activate_render_layer_sprite_batch(const int layer_index, const int batch_index,
                                                              zfw_sprite_batch_group_t *const batch_group,
                                                              zfw_mem_arena_t *const main_mem_arena)
{
    ...
    {
        float *verts;
        const int verts_size =
            sizeof(*verts) * ZFW_BUILTIN_SPRITE_QUAD_SHADER_PROG_VERT_COUNT * 4 * ZFW_SPRITE_BATCH_SLOT_LIMIT;
...
static void draw_sprite_batches_of_layer(const zfw_sprite_batch_group_t *const batch_group, const int layer_index,
                                         const zfw_user_tex_data_t *const user_tex_data,
        ...
        for (int j = 0; j < ZFW_SPRITE_BATCH_TEX_UNIT_LIMIT; j++)
        {
            ...
            if (batch_group->tex_units[batch_group_tex_unit_index].count)
            {
                ...
                glBindTexture(GL_TEXTURE_2D,
                              user_tex_data->gl_ids[batch_group->tex_units[batch_group_tex_unit_index].user_tex_index]);
            }
        }

        glUniform1iv(glGetUniformLocation(builtin_shader_prog_data->sprite_quad_prog_gl_id, "u_textures"),
                     ZFW_STATIC_ARRAY_LEN(tex_units), tex_units);
...
static void draw_char_batches_of_layer(const zfw_char_batch_group_t *const batch_group, const int layer_index,
                                       const zfw_user_font_data_t *const user_font_data,
                                       const zfw_builtin_shader_prog_data_t *const builtin_shader_prog_data)
{
    if (!batch_group->batch_activity_bits[layer_index])
    {
    ...
    for (int i = 0; i < ZFW_RENDER_LAYER_CHAR_BATCH_LIMIT; i++)
    {
        ...
        glUniform2fv(glGetUniformLocation(builtin_shader_prog_data->char_quad_prog_gl_id, "u_pos"), 1,
                     (float *)&batch_group->positions[batch_group_batch_index]);
        glUniform2fv(glGetUniformLocation(builtin_shader_prog_data->char_quad_prog_gl_id, "u_scale"), 1,
                     (float *)&batch_group->scales[batch_group_batch_index]);
        glUniform4fv(glGetUniformLocation(builtin_shader_prog_data->char_quad_prog_gl_id, "u_blend"), 1,
                     (float *)&batch_group->blends[batch_group_batch_index]);

        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D,
                      user_font_data->tex_gl_ids[batch_group->user_font_indexes[batch_group_batch_index]]);
...
zfw_bool_t zfw_init_sprite_batch_group(zfw_sprite_batch_group_t *const batch_group,
                                       zfw_mem_arena_t *const main_mem_arena)
{
    ...
    batch_group->vert_array_gl_ids =
        zfw_mem_arena_alloc(main_mem_arena, sizeof(*batch_group->vert_array_gl_ids) * batch_group_batch_count);
    ...
    batch_group->vert_buf_gl_ids =
        zfw_mem_arena_alloc(main_mem_arena, sizeof(*batch_group->vert_buf_gl_ids) * batch_group_batch_count);
    ...
    batch_group->elem_buf_gl_ids =
        zfw_mem_arena_alloc(main_mem_arena, sizeof(*batch_group->elem_buf_gl_ids) * batch_group_batch_count);
    ...
    batch_group->tex_units = zfw_mem_arena_alloc(
        main_mem_arena, sizeof(*batch_group->tex_units) * ZFW_SPRITE_BATCH_TEX_UNIT_LIMIT * batch_group_batch_count);
    ...
    zfw_init_bitset_in_mem_arena(
        &batch_group->slot_activity,
        ZFW_SPRITE_BATCH_SLOT_LIMIT * ZFW_RENDER_LAYER_SPRITE_BATCH_LIMIT * ZFW_RENDER_LAYER_LIMIT, main_mem_arena);
    //
...
void zfw_clean_sprite_batch_group(zfw_sprite_batch_group_t *const batch_group)
{
    ...
    if (batch_group->vert_array_gl_ids)
    {
        glDeleteVertexArrays(ZFW_RENDER_LAYER_SPRITE_BATCH_LIMIT * ZFW_RENDER_LAYER_LIMIT,
                             batch_group->vert_array_gl_ids);
    }
}
...
void zfw_set_sprite_batch_group_defaults(zfw_sprite_batch_group_t *const batch_group)
{
    ...
    memset(batch_group->tex_units, 0,
           sizeof(*batch_group->tex_units) * ZFW_SPRITE_BATCH_TEX_UNIT_LIMIT * ZFW_RENDER_LAYER_SPRITE_BATCH_LIMIT *
               ZFW_RENDER_LAYER_LIMIT);
}

zfw_sprite_batch_slot_key_t zfw_take_render_layer_sprite_batch_slot(
    const zfw_sprite_batch_group_id_t batch_group_id, const int layer_index, const int user_tex_index,
    zfw_sprite_batch_group_t batch_groups[ZFW_SPRITE_BATCH_GROUP_COUNT], zfw_mem_arena_t *const main_mem_arena)
{
    ...
    for (int i = 0; i < ZFW_RENDER_LAYER_SPRITE_BATCH_LIMIT; i++)
    {
        ...
        if (!(batch_groups[batch_group_id].batch_activity_bits[layer_index] &
              ((zfw_render_layer_sprite_batch_activity_bits_t)1 << i)))
        {
        ...
        {
            const zfw_sprite_batch_tex_unit_t tex_unit =
                batch_groups[batch_group_id].tex_units[zfw_get_sprite_batch_group_tex_unit_index(layer_index, i, j)];
        ...
        const int slot_activity_bitset_begin_bit_index =
            (layer_index * ZFW_RENDER_LAYER_SPRITE_BATCH_LIMIT * ZFW_SPRITE_BATCH_SLOT_LIMIT) +
            (i * ZFW_SPRITE_BATCH_SLOT_LIMIT);
        const int slot_activity_bitset_first_inactive_bit_index = zfw_get_first_inactive_bitset_bit_index_in_range(
            &batch_groups[batch_group_id].slot_activity, slot_activity_bitset_begin_bit_index,
            slot_activity_bitset_begin_bit_index + ZFW_SPRITE_BATCH_SLOT_LIMIT);

        if (slot_activity_bitset_first_inactive_bit_index != -1)
        {
            // Take the slot and return a key.
            zfw_activate_bitset_bit(&batch_groups[batch_group_id].slot_activity,
                                    slot_activity_bitset_first_inactive_bit_index);

            const int batch_group_tex_unit_index =
                zfw_get_sprite_batch_group_tex_unit_index(layer_index, i, tex_unit_index);
            ...
            slot_key_elems[ZFW_SPRITE_BATCH_SLOT_KEY_ELEM_ID__SLOT_INDEX] =
                slot_activity_bitset_first_inactive_bit_index - slot_activity_bitset_begin_bit_index;
    ...
    if (first_inactive_batch_index != -1)
    {
        ...
        if (init_and_activate_render_layer_sprite_batch(layer_index, first_inactive_batch_index,
                                                        &batch_groups[batch_group_id], main_mem_arena))
        {
            return zfw_take_render_layer_sprite_batch_slot(batch_group_id, layer_index, user_tex_index, batch_groups,
                                                           main_mem_arena);
        }
    }
...
void zfw_take_multiple_render_layer_sprite_batch_slots(
    zfw_sprite_batch_slot_key_t *const slot_keys, const int slot_key_count,
    const zfw_sprite_batch_group_id_t batch_group_id, const int layer_index, const int user_tex_index,
    zfw_sprite_batch_group_t batch_groups[ZFW_SPRITE_BATCH_GROUP_COUNT], zfw_mem_arena_t *const main_mem_arena)
{
    if (slot_key_count <= 0)
    {
        zfw_log_warning("Invalid \"slot_key_count\" parameter value (%d) for function "
                        "\"zfw_take_multiple_slots_from_render_layer_sprite_batch\".",
                        slot_key_count);
        return;
    }
    ...
    for (int i = 0; i < ZFW_RENDER_LAYER_SPRITE_BATCH_LIMIT; i++)
    {
        ...
        const int slot_activity_bitset_begin_bit_index =
            (ZFW_SPRITE_BATCH_SLOT_LIMIT * ZFW_RENDER_LAYER_SPRITE_BATCH_LIMIT * layer_index) +
            (i * ZFW_SPRITE_BATCH_SLOT_LIMIT);
        const int batch_group_tex_unit_index =
            zfw_get_sprite_batch_group_tex_unit_index(layer_index, i, tex_unit_index);
        ...
        {
            if (!zfw_is_bitset_bit_active(&batch_groups[batch_group_id].slot_activity,
                                          slot_activity_bitset_begin_bit_index + j))
            {
    ...
    if (first_inactive_batch_index != -1)
    {
        ...
        if (init_and_activate_render_layer_sprite_batch(layer_index, first_inactive_batch_index,
                                                        &batch_groups[batch_group_id], main_mem_arena))
        {
            zfw_take_multiple_render_layer_sprite_batch_slots(
                slot_keys + slots_found_count, slot_key_count - slots_found_count, batch_group_id, layer_index,
                user_tex_index, batch_groups, main_mem_arena);
        }
    }
}

zfw_bool_t zfw_write_to_render_layer_sprite_batch_slot(
    const zfw_sprite_batch_slot_key_t slot_key, const zfw_vec_2d_t pos, const float rot, const zfw_vec_2d_t scale,
    const zfw_vec_2d_t origin, const zfw_rect_t *const src_rect, const zfw_color_t *const blend,
    const zfw_sprite_batch_group_t batch_groups[ZFW_SPRITE_BATCH_GROUP_COUNT],
    const zfw_user_tex_data_t *const user_tex_data)
{
    ...
    const zfw_sprite_batch_group_t *const batch_group =
        &batch_groups[slot_key_elems[ZFW_SPRITE_BATCH_SLOT_KEY_ELEM_ID__BATCH_GROUP_INDEX]];

    const int user_tex_index = batch_group
                                   ->tex_units[zfw_get_sprite_batch_group_tex_unit_index(
                                       slot_key_elems[ZFW_SPRITE_BATCH_SLOT_KEY_ELEM_ID__LAYER_INDEX],
                                       slot_key_elems[ZFW_SPRITE_BATCH_SLOT_KEY_ELEM_ID__BATCH_INDEX],
    ...
    const int batch_group_batch_index =
        zfw_get_sprite_batch_group_batch_index(slot_key_elems[ZFW_SPRITE_BATCH_SLOT_KEY_ELEM_ID__LAYER_INDEX],
                                               slot_key_elems[ZFW_SPRITE_BATCH_SLOT_KEY_ELEM_ID__BATCH_INDEX]);
    glBindVertexArray(batch_group->vert_array_gl_ids[batch_group_batch_index]);
    glBindBuffer(GL_ARRAY_BUFFER, batch_group->vert_buf_gl_ids[batch_group_batch_index]);
    glBufferSubData(GL_ARRAY_BUFFER, slot_key_elems[ZFW_SPRITE_BATCH_SLOT_KEY_ELEM_ID__SLOT_INDEX] * sizeof(verts),
                    sizeof(verts), verts);

    return ZFW_TRUE;
}

zfw_bool_t zfw_clear_render_layer_sprite_batch_slot(
    const zfw_sprite_batch_slot_key_t slot_key,
    const zfw_sprite_batch_group_t batch_groups[ZFW_SPRITE_BATCH_GROUP_COUNT])
{
    if (!zfw_is_sprite_batch_slot_key_active(slot_key))
    {
...
zfw_bool_t zfw_free_render_layer_sprite_batch_slot(const zfw_sprite_batch_slot_key_t slot_key,
                                                   zfw_sprite_batch_group_t batch_groups[ZFW_SPRITE_BATCH_GROUP_COUNT])
{
    if (!zfw_is_sprite_batch_slot_key_active(slot_key))
    {
    ...
    zfw_deactivate_bitset_bit(
        &batch_group->slot_activity,
        zfw_get_sprite_batch_group_slot_index(slot_key_elems[ZFW_SPRITE_BATCH_SLOT_KEY_ELEM_ID__LAYER_INDEX],
                                              slot_key_elems[ZFW_SPRITE_BATCH_SLOT_KEY_ELEM_ID__BATCH_INDEX],
                                              slot_key_elems[ZFW_SPRITE_BATCH_SLOT_KEY_ELEM_ID__SLOT_INDEX]));
...
int zfw_get_sprite_batch_slot_key_elem(const zfw_sprite_batch_slot_key_t slot_key,
                                       const zfw_sprite_batch_slot_key_elem_id_t elem_id)
{
...
void zfw_init_sprite_batch_slot_key_elems(const zfw_sprite_batch_slot_key_t slot_key,
                                          int key_elems[ZFW_SPRITE_BATCH_SLOT_KEY_ELEM_COUNT])
{
    int bit_index = 1; // (Skip the activity bit.)
...
zfw_bool_t zfw_init_char_batch_group(zfw_char_batch_group_t *const batch_group, zfw_mem_arena_t *const main_mem_arena)
{
    ...
    batch_group->user_font_indexes =
        zfw_mem_arena_alloc(main_mem_arena, sizeof(*batch_group->user_font_indexes) * batch_group_batch_count);
    ...
    batch_group->positions =
        zfw_mem_arena_alloc(main_mem_arena, sizeof(*batch_group->positions) * batch_group_batch_count);
...
zfw_char_batch_key_t zfw_take_render_layer_char_batch(const int layer_index, zfw_char_batch_group_t *const batch_group,
                                                      zfw_mem_arena_t *const main_mem_arena)
{
    for (int i = 0; i < ZFW_RENDER_LAYER_CHAR_BATCH_LIMIT; i++)
    {
        ...
        if (!(batch_group->batch_activity_bits[layer_index] & batch_bitmask))
        {
            ...
            if (!(batch_group->batch_init_bits[layer_index] & batch_bitmask))
            {
...

                {
                    float *verts;
                    const int verts_size =
                        sizeof(*verts) * ZFW_BUILTIN_CHAR_QUAD_SHADER_PROG_VERT_COUNT * 4 * ZFW_CHAR_BATCH_SLOT_LIMIT;
                    verts = zfw_mem_arena_alloc(main_mem_arena, verts_size);

                    if (!verts)
                    {
                        zfw_log_error("Failed to allocate %d bytes for render layer character batch vertices!",
                                      verts_size);
                        return 0;
                    }
...

                {
                    ...
                    if (!indices)
                    {
                        zfw_log_error("Failed to allocate %d bytes for render layer character batch elements!",
                                      indices_size);
                        return 0;
                    }
...
zfw_bool_t zfw_write_to_render_layer_char_batch(const zfw_char_batch_key_t key, const char *const text,
                                                const zfw_font_hor_align_t hor_align,
                                                const zfw_font_vert_align_t vert_align,
                                                zfw_char_batch_group_t *const batch_group,
                                                const zfw_user_font_data_t *const user_font_data)
{
    ...
    const int batch_group_batch_index = zfw_get_char_batch_group_batch_index(
        zfw_get_char_batch_slot_key_layer_index(key), zfw_get_char_batch_slot_key_batch_index(key));
    ...
    {
        zfw_log_warning(
            "Attempting to write to a render layer character batch using text of a length exceeding the limit of %d!",
            ZFW_CHAR_BATCH_SLOT_LIMIT);
        return ZFW_FALSE;
    }

    // Determine the positions of text characters based on font information, alongside the overall dimensions of the
    // text to be used when applying alignment.
    ...
    for (int i = 0; i < text_len; i++)
    {
        if (text[i] == '\n')
        {
            ...
            if (!text_first_line_min_offs_updated)
            {
                ...
                text_first_line_min_offs =
                    user_font_data->chars_vert_offsets[batch_user_font_index * ZFW_FONT_CHAR_RANGE_SIZE];
                text_first_line_min_offs_updated = ZFW_TRUE;
            }

            // Set the last line maximum h to the h of a space.
            text_last_line_max_h =
                user_font_data->chars_vert_offsets[batch_user_font_index * ZFW_FONT_CHAR_RANGE_SIZE] +
                user_font_data->chars_src_rects[batch_user_font_index * ZFW_FONT_CHAR_RANGE_SIZE].h;
            //
        ...
        if (text_line_counter == 0)
        {
            ...
            {
                text_first_line_min_offs =
                    ZFW_MIN(user_font_data->chars_vert_offsets[user_font_chars_index], text_first_line_min_offs);
            }
        }

        if (!text_last_line_max_h_updated)
        {
            text_last_line_max_h = user_font_data->chars_vert_offsets[user_font_chars_index] +
                                   user_font_data->chars_src_rects[user_font_chars_index].h;
            text_last_line_max_h_updated = ZFW_TRUE;
        }
        else
        {
            text_last_line_max_h = ZFW_MAX(user_font_data->chars_vert_offsets[user_font_chars_index] +
                                               user_font_data->chars_src_rects[user_font_chars_index].h,
                                           text_last_line_max_h);
        }
        ...
        if (i > 0)
        {
            ...
            char_draw_pos_pen.x +=
                user_font_data
                    ->chars_kernings[(batch_user_font_index * ZFW_FONT_CHAR_RANGE_SIZE * ZFW_FONT_CHAR_RANGE_SIZE) +
                                     (text_char_index * ZFW_FONT_CHAR_RANGE_SIZE) + text_char_index_last];
        }
    ...
    {
        const float batch_clear_verts[(ZFW_BUILTIN_CHAR_QUAD_SHADER_PROG_VERT_COUNT * 4) * ZFW_CHAR_BATCH_SLOT_LIMIT] =
            {0};
    ...
    for (int i = 0; i < text_len; i++)
    {
        ...
        if (text[i] != ' ')
        {
            const int font_chars_index =
                (batch_user_font_index * ZFW_FONT_CHAR_RANGE_SIZE) + (text[i] - ZFW_FONT_CHAR_RANGE_BEGIN);

            const zfw_vec_2d_t char_draw_pos =
                zfw_create_vec_2d(char_draw_positions[i].x - (text_line_ws[text_line_counter] * (hor_align / 2.0f)),
                                  char_draw_positions[i].y - (text_h * (vert_align / 2.0f)));

            zfw_rect_f_t char_tex_coords_rect;
            char_tex_coords_rect.x = (float)user_font_data->chars_src_rects[font_chars_index].x /
                                     user_font_data->tex_sizes[batch_user_font_index].x;
            char_tex_coords_rect.y = (float)user_font_data->chars_src_rects[font_chars_index].y /
                                     user_font_data->tex_sizes[batch_user_font_index].y;
            char_tex_coords_rect.w = (float)user_font_data->chars_src_rects[font_chars_index].w /
                                     user_font_data->tex_sizes[batch_user_font_index].x;
            char_tex_coords_rect.h = (float)user_font_data->chars_src_rects[font_chars_index].h /
                                     user_font_data->tex_sizes[batch_user_font_index].y;
...
void zfw_render_sprite_and_character_batches(
    const zfw_sprite_batch_group_t sprite_batch_groups[ZFW_SPRITE_BATCH_GROUP_COUNT],
    const zfw_char_batch_group_t *const char_batch_group, const zfw_view_state_t *const view_state,
    const zfw_vec_2d_i_t window_size, const zfw_user_tex_data_t *const user_tex_data,
    const zfw_user_font_data_t *const user_font_data,
    const zfw_builtin_shader_prog_data_t *const builtin_shader_prog_data)
{
    ...
    // Draw view sprite batches.
    {
        ...
        glUniformMatrix4fv(glGetUniformLocation(builtin_shader_prog_data->sprite_quad_prog_gl_id, "u_view"), 1,
                           GL_FALSE, (float *)view.elems);

        glUniformMatrix4fv(glGetUniformLocation(builtin_shader_prog_data->sprite_quad_prog_gl_id, "u_proj"), 1,
                           GL_FALSE, (float *)proj.elems);

        for (int i = 0; i < ZFW_RENDER_LAYER_LIMIT; i++)
        {
            draw_sprite_batches_of_layer(&sprite_batch_groups[ZFW_SPRITE_BATCH_GROUP_ID__VIEW], i, user_tex_data,
    ...
    for (int i = 0; i < ZFW_RENDER_LAYER_LIMIT; i++)
    {
        ...
        draw_sprite_batches_of_layer(&sprite_batch_groups[ZFW_SPRITE_BATCH_GROUP_ID__SCREEN], i, user_tex_data,
                                     builtin_shader_prog_data);

        // Draw layer character batches.
        glUseProgram(builtin_shader_prog_data->char_quad_prog_gl_id);
        glUniformMatrix4fv(glGetUniformLocation(builtin_shader_prog_data->char_quad_prog_gl_id, "u_proj"), 1, GL_FALSE,
                           (float *)proj.elems);
