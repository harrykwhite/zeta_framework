...
static zfw_bool_t retrieve_user_asset_data_from_assets_file(zfw_user_asset_data_t *const user_asset_data, FILE *const assets_file_fs)
{
    ...
    if (user_asset_data->font_data.font_count)
    {
        // Allocate memory for and retrieve font line heights.
        {
            ...
            user_asset_data->font_data.line_heights = zfw_mem_arena_alloc(&zfw_g_main_mem_arena, sizeof(*user_asset_data->font_data.line_heights) * line_height_count);
        ...
        // Allocate memory for and retrieve font character horizontal offsets.
        {
            ...
            user_asset_data->font_data.chars_hor_offsets = zfw_mem_arena_alloc(&zfw_g_main_mem_arena, sizeof(*user_asset_data->font_data.chars_hor_offsets) * hor_offs_count);
        ...
        // Allocate memory for and retrieve font character vertical offsets.
        {
            ...
            user_asset_data->font_data.chars_vert_offsets = zfw_mem_arena_alloc(&zfw_g_main_mem_arena, sizeof(*user_asset_data->font_data.chars_vert_offsets) * vert_offs_count);
        ...
        // Allocate memory for and retrieve font character horizontal advances.
        {
            ...
            user_asset_data->font_data.chars_hor_advances = zfw_mem_arena_alloc(&zfw_g_main_mem_arena, sizeof(*user_asset_data->font_data.chars_hor_advances) * hor_advance_count);
        ...
        // Allocate memory for and retrieve font character source rectangles.
        {
            ...
            user_asset_data->font_data.chars_src_rects = zfw_mem_arena_alloc(&zfw_g_main_mem_arena, sizeof(*user_asset_data->font_data.chars_src_rects) * src_rect_count);
        ...
        // Allocate memory for and retrieve font character kernings.
        {
            ...
            user_asset_data->font_data.chars_kernings = zfw_mem_arena_alloc(&zfw_g_main_mem_arena, sizeof(*user_asset_data->font_data.chars_kernings) * kerning_count);
        ...
        // Allocate memory for and retrieve font texture sizes.
        {
            ...
            user_asset_data->font_data.tex_sizes = zfw_mem_arena_alloc(&zfw_g_main_mem_arena, sizeof(*user_asset_data->font_data.tex_sizes) * tex_size_count);
        ...
        user_asset_data->font_data.tex_glids = zfw_mem_arena_alloc(&zfw_g_main_mem_arena, sizeof(*user_asset_data->font_data.tex_glids) * user_asset_data->font_data.font_count);
...
static void draw_sprite_batches_of_layer(const int layer_index, zfw_sprite_batch_data_t *const batch_data, const zfw_user_tex_data_t *const user_tex_data, const zfw_builtin_shader_prog_data_t *const builtin_shader_prog_data)
{
    if (!batch_data->batch_activity_bits[layer_index])
    {
    ...
    {
        if (!(batch_data->batch_activity_bits[layer_index] & ((zfw_render_layer_sprite_batch_activity_bits_t)1 << i)))
        {
            continue;
        }
        ...
        {
            const int user_tex_index = batch_data->tex_units[layer_index][i][j].user_tex_index;
...
static void init_char_batch_data(zfw_char_batch_data_t *const batch_data)
{
    glGenVertexArrays(ZFW_RENDER_LAYER_CHAR_BATCH_LIMIT * ZFW_RENDER_LAYER_LIMIT, (GLuint *)batch_data->vert_array_gl_ids);
    glGenBuffers(ZFW_RENDER_LAYER_CHAR_BATCH_LIMIT * ZFW_RENDER_LAYER_LIMIT, (GLuint *)batch_data->vert_buf_gl_ids);
    glGenBuffers(ZFW_RENDER_LAYER_CHAR_BATCH_LIMIT * ZFW_RENDER_LAYER_LIMIT, (GLuint *)batch_data->elem_buf_gl_ids);
}

static void draw_char_batches_of_layer(const int layer_index, zfw_char_batch_data_t *const batch_data, const zfw_user_font_data_t *const user_font_data, const zfw_builtin_shader_prog_data_t *const builtin_shader_prog_data)
{
    if (!batch_data->batch_activity_bits[layer_index])
    {
        // All the batches of this layer are inactive, so don't bother proceeding.
    ...
    {
        if (!(batch_data->batch_activity_bits[layer_index] & ((zfw_render_layer_char_batch_bits_t)1 << i)))
        {
            continue;
        }
...
static void clean_char_batch_data(zfw_char_batch_data_t *const batch_data)
{
    glDeleteBuffers(ZFW_RENDER_LAYER_CHAR_BATCH_LIMIT * ZFW_RENDER_LAYER_LIMIT, (GLuint *)batch_data->elem_buf_gl_ids);
    glDeleteBuffers(ZFW_RENDER_LAYER_CHAR_BATCH_LIMIT * ZFW_RENDER_LAYER_LIMIT, (GLuint *)batch_data->vert_buf_gl_ids);
    glDeleteVertexArrays(ZFW_RENDER_LAYER_CHAR_BATCH_LIMIT * ZFW_RENDER_LAYER_LIMIT, (GLuint *)batch_data->vert_array_gl_ids);
}
...
static void clean_game(game_cleanup_data_t *const cleanup_data)
{
    ...
    {
        clean_char_batch_data(cleanup_data->char_batch_data);
    }
...
zfw_bool_t zfw_run_game(const zfw_user_game_run_info_t *const user_run_info)
{
    ...
    while (ZFW_TRUE)
    {
        ...
        // Run the user-defined game initialization function.
        {
            ...
            init_data.char_batch_data = &char_batch_data;
        ...
        while (!(glfw_window_should_close = glfwWindowShouldClose(glfw_window)) && !restart)
        {
            ...
            if (user_run_info->on_window_resize_func && (window_state.size.x != window_size_last.x || window_state.size.y != window_size_last.y))
            {
                ...
                window_resize_data.char_batch_data = &char_batch_data;
            ...
            {
                {
                    ...
                    tick_data.char_batch_data = &char_batch_data;
            ...
            if (tick_count > 0)
            {
                ...
                // Draw view sprite batches.
                {
                    ...
                    for (int i = 0; i < ZFW_RENDER_LAYER_LIMIT; i++)
                    {
                        draw_sprite_batches_of_layer(i, &sprite_batch_datas[ZFW_SPRITE_BATCH_DATA_ID__VIEW], &user_asset_data.tex_data, &builtin_shader_prog_data);
                    }
                }
